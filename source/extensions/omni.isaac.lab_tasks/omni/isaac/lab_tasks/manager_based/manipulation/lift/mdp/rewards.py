...
def object_is_lifted(
    ...
    if isinstance(env.obs_buf["policy"], torch.Tensor):
        return torch.where(object.data.root_pos_w[:, 2] > minimal_height, 1.0, 0.0) * torch.where(env.obs_buf["policy"][:, -1]  == 1.0, -1.0, 1.0)
...
def stay_low(
    ...
    """Reward the agent for dropping the object below the minimal height, after having lifted it."""
    object: RigidObject = env.scene[object_cfg.name]
